...
pkill -f 'nodeharvest-server' 2>/dev/null || true
pkill -f 'next dev.*nodeharvest|/web/node_modules/next|next dev -H' 2>/dev/null || true
# 更精确：cwd 在本项目下的 next
pkill -f "${ROOT}/web" 2>/dev/null || true

echo "NodeHarvest stopped (API :${API_PORT} + Next.js)"
